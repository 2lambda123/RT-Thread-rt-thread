...
rt_err_t rt_pci_ofw_device_init(struct rt_pci_device *pdev)
{
    ...
    if (rt_pci_is_root_bus(pdev->bus))
    {
        struct rt_pci_device *first_pdev;

        np = pdev->bus->host_bridge->parent.ofw_node;

        if (!np)
        {
            goto _ofw_node_set_end;
        }

        rt_hw_spin_lock(&pdev->bus->lock.lock);
        /* The first PCI device is most likely a bridge */
        first_pdev = rt_list_first_entry(&pdev->bus->devices_nodes,
                struct rt_pci_device, list);
        rt_hw_spin_unlock(&pdev->bus->lock.lock);

        if (!first_pdev->parent.ofw_node)
        {
            if (first_pdev == pdev)
            {
                /* If NULL, the ofw node is not support on this bus */
                np = ofw_find_device(np, pdev->devfn);
                first_pdev->parent.ofw_node = np;
            }
            else
            {
                np = RT_NULL;
